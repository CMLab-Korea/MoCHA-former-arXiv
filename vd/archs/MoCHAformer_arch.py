...
from vd.archs.freq_fft_utils import FreqDecoupleBlock5
from vd.archs.MPDN_Vis_arch import MPDM_ver3
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, x_size,attn_mask):
        ...
        x = shortcut + self.drop_path(x) + fca_x * self.fca_w
        # (2) LayerNorm
        ffn_in = self.norm2(x)
        # (3) MLP 통과
...
class RSTB(nn.Module):
    ...
    def __init__(self,
        ...
        self.patch_unembed = PatchUnEmbed(
            img_size=img_size, patch_size=patch_size, in_chans=0, embed_dim=dim, norm_layer=None)
        self.fdb = FreqDecoupleBlock5(channels=dim, window_size=window_size[1])
...
class MoCHAformer(nn.Module):
    ...
    def __init__(self,
                 ...
                 num_frames=3,
                 **kwargs):
        super(MoCHAformer, self).__init__()
        ...
        self.stage = stage
        if self.stage == 2:
            self.mpdm = MPDM_ver3(embed_dim=96, num_frames=3, convnext_block_c=convnext_block_c, convnext_block_m=convnext_block_m, stage=stage)
    ...
    def forward(self, x, phase='train', is_train=True):
        ...
        ## vis part #######################################################################################
        if vis_state:
            store_vis_data(out_vis, '01_lq_raw', rearrange(x, 'b t c h w -> b c t h w'), 'raw', num_flow=1)
        ###################################################################################################
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            if c == 4:
                ...
                if self.stage == 2:
                    ## vis part #######################################################################################
                    if vis_state:
                        coarse_feat, moire_pat_out, clean_inter_frm_out, clean_inter_frm_out_rgb, vis_dict = self.mpdm(x, vis_state)
                        out_vis.update(vis_dict)
                    ####################################################################################################
                    else:
                        coarse_feat, moire_pat_out, clean_inter_frm_out, clean_inter_frm_out_rgb = self.mpdm(x, vis_state)
                    x = coarse_feat + x
            ...
            if self.stage == 2:
                moire_pat_out = rearrange(moire_pat_out, '(b t) c h w -> b t c h w', c=self.raw_ch, t=self.num_frames)
                clean_inter_frm_out = rearrange(clean_inter_frm_out, '(b t) c h w -> b t c h w', c=self.raw_ch, t=self.num_frames)
