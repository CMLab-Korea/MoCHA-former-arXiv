...
from vd.archs.freq_utils_arch import RFFTChannelAttention
from vd.archs.freq_fft_utils import FreqDecoupleBlock4
from vd.archs.MPDN_Vis_arch import DMAD_rgb
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.fca = RFFTChannelAttention(channels=dim, reduction=16)
        # self.fca_scale = nn.Parameter(torch.tensor(0.01, dtype=torch.float32))
        self.fca_scale = 0.01
    ...
    def forward(self, x, x_size,attn_mask):
        ...
        x = self.norm1(x) # b t h w c
        fca_x = rearrange(x, 'b t h w c -> (b t) c h w', b=b, t=t)
        fca_x = self.fca(fca_x)  # (b*t, c, h, w)
        fca_x = rearrange(fca_x, '(b t) c h w -> b t h w c', b=b, t=t)
        ...
        # (1) 첫 번째 residual + DropPath
        x = shortcut + self.drop_path(x) + fca_x * self.fca_scale
        # (2) LayerNorm
        ffn_in = self.norm2(x)
        # (3) MLP 통과
...
class RSTB(nn.Module):
    ...
    def __init__(self,
        ...
        self.patch_unembed = PatchUnEmbed(
            img_size=img_size, patch_size=patch_size, in_chans=0, embed_dim=dim, norm_layer=None)
        self.fdb = FreqDecoupleBlock4(channels=dim, window_size=window_size[1])
...
class ShallowFeatExtractor(nn.Module):
    def __init__(self, in_ch, out_ch):
        super(ShallowFeatExtractor, self).__init__()
        self.conv1 = nn.Conv3d(in_ch, out_ch // 2, kernel_size=(1, 3, 3), stride=(1,1,1), padding=(0, 1, 1))
        self.relu = nn.ReLU()
        self.conv2 = nn.Conv3d(out_ch // 2, out_ch, kernel_size=(1, 3, 3),stride=(1,1,1), padding=(0, 1, 1))
...
class MoCHAformer_rgb(nn.Module):
    r""" MoCHAformer: Moiré-Conditioned Hybrid-Adaptive Transformer (MoCHAformer)
        MoCHAformer = STAD + DMAD
            STAD: Spatio-Temporal Adaptive Demoiréing module
            DMAD: Decoupled Moire-Adaptive Demoiréing module

        stage 1: only STAD
        stage 2: STAD + DMAD (MPDM)
    """
    ...
    def __init__(self,
                 ...
                 num_frames=3,
                 **kwargs):
        super(MoCHAformer_rgb, self).__init__()
        num_in_ch = in_chans  #4
        num_out_ch = in_chans  #4
        num_feat = 64
        ...
        # downscale -> upscale pipeline for efficient gpu usage of VDemoire dataset
        # Pixel shuffle -> conv -> conv -> Pixel unshuffle
        self.ps = nn.PixelUnshuffle(downscale_factor=2) 
        self.conv_ds = nn.Conv2d(num_in_ch * 4, embed_dim, 3, 1, 1)
        self.conv_us = nn.Conv2d(embed_dim, num_in_ch * 4, 3, 1, 1)
        self.pus = nn.PixelShuffle(upscale_factor=2)
        ...
        self.feat_ext = ShallowFeatExtractor(in_ch=4, out_ch=embed_dim)
        ...
        #                     nn.Conv2d(in_channels=embed_dim // 2, out_channels=3, kernel_size=3, stride=1, padding=1)) # 3x3 conv -> RGB 3 channels
        
        self.rgb_ch = 3
        self.raw_ch = 4
        self.stage = stage
        if self.stage == 2:
            self.dmad = DMAD_rgb(embed_dim=96, num_frames=3, convnext_block_c=convnext_block_c, convnext_block_m=convnext_block_m, stage=stage)
    ...
    def forward(self, x, phase='train', is_train=True):
        n, t, c, h, w = x.size()
        
        if phase == 'train': pass   
        else: pass
        
        if phase != 'train' and is_train == False: vis_state = True
        else: vis_state = False
        if self.stage == 1:
            out_vis = {
                # input lq frames
                '01_gt_rgb': None,
                '01_lq_raw': None,
                '01_coarse_clean_rgb': None,
                '02_pred_clean_rgb': None,
            }
        elif self.stage == 2:
            out_vis = {
                # input lq frames
                '01_gt_rgb': None,
                '01_lq_raw': None,
                '01_coarse_clean_raw': None,
                '01_coarse_clean_rgb': None,
                '01_moire_pat_raw': None,
        ...
        ## vis part #######################################################################################
        if vis_state:
            store_vis_data(out_vis, '01_lq_rgb', rearrange(x, 'b t c h w -> b c t h w'), 'rgb', num_flow=1)
        ###################################################################################################
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            # x = self.conv_first(x) # bt c h w
            x = self.ps(x) # bt 3 h w -> bt 12 h/2 w/2
            x = self.conv_ds(x) # bt 12 h/2 w/2 -> bt embed_dim h/2 w/2
            
            # stage 2 -> applying MDPM
            if self.stage == 2:
                ## vis part #######################################################################################
                if vis_state:
                    coarse_feat, moire_pat_out, clean_inter_frm_out, clean_inter_frm_out_rgb, vis_dict = self.dmad(x, vis_state)
                    out_vis.update(vis_dict)
                ####################################################################################################
                else:
                    coarse_feat, moire_pat_out, clean_inter_frm_out, clean_inter_frm_out_rgb = self.dmad(x, vis_state)
                x = coarse_feat + x
            ...
            x = self.conv_after_body(feats[:, t // 2, :, :, :]) + x_center # [B C H W]/[B 48 H W]
            x = self.conv_us(x)  # b c h w -> b 12 t h w
            x = self.pus(x)  # b 12 h/2 w/2 -> b 3 h w
            # x = self.conv_isp(x)
            
            if self.stage == 2:
                moire_pat_out = rearrange(moire_pat_out, '(b t) c h w -> b t c h w', c=self.rgb_ch, t=self.num_frames)
                clean_inter_frm_out = rearrange(clean_inter_frm_out, '(b t) c h w -> b t c h w', c=self.rgb_ch, t=self.num_frames)
                clean_inter_frm_out_rgb = rearrange(clean_inter_frm_out_rgb, '(b t) c h w -> b t c h w', c=self.rgb_ch, t=self.num_frames)
            
        if self.stage == 1:
            return x
        elif self.stage == 2:
            return x, moire_pat_out, clean_inter_frm_out, clean_inter_frm_out_rgb, out_vis
